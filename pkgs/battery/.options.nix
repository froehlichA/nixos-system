      ...
      script = ''
        ...
        if [[ $battery_capacity -le ${
          builtins.toString cfg.notifyCapacity
            ...
            ${pkgs.libnotify}/bin/notify-send --urgency=critical --hint=int:transient:1 --icon=battery "Battery Low" "You should probably plug in."
        fi
        if [[ $battery_capacity -le ${
          builtins.toString cfg.suspendCapacity
        } && $battery_status = "Discharging" ]]; then
            ${pkgs.libnotify}/bin/notify-send --urgency=critical --hint=int:transient:1 --icon=battery "Battery Critically Low" "Computer will suspend in 60 seconds."
            sleep 60s
